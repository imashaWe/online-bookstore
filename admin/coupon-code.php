...
$sql = "SELECT count(id) AS count  FROM coupon_code WHERE is_delete = 0";
...
if (isset($_POST['delete_submit'])) {
    $id = $_POST['id'];
    $sql = "UPDATE coupon_code SET is_delete = 1 WHERE id = {$id}";
    $res = $conn->query($sql);
    ...
    <main>
        <div class="container-fluid px-4">
            ...
            <div class="d-grid  justify-content-end pb-2">
                <a href="coupon-code-add.php" class="btn btn-dark">Add New Code</a>
            </div>
            <div class="card">
                    ...
                    <table class="table table-striped">
                        ...
                        <?php while ($coupon = $coupons->fetch_array()): ?>
                            <tr>
                                ...
                                <td><?= $coupon['to'] ?></td>
                                <td class="text-end">
                                    <a href="coupon-code-add.php?id=<?= $coupon['id'] ?>"
                                       class="btn btn-secondary">Edit</a>
                                    <button type="button"
                                            class="btn btn-danger btn-user-delete"
                                            data-bs-toggle="modal"
                                            data-bs-target="#deleteModal"
                                            onclick="setDelete(<?= $coupon['id'] ?>)"
                                    >
                                        Delete
                                    </button>
                                </td>
